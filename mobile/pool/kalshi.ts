// Live "odds to cover" from Kalshi's public market data — mirrors
// web/src/pool/kalshi.ts (see CLAUDE.md parity rule). Native fetch has no
// CORS, so mobile hits Kalshi's API directly instead of going through the
// proxy the browser needs.
...
export async function fetchCoverOddsForSlate(
  ...
  try {
    ...
    return out; // network down — odds are optional
  }
